                    ...
                    <v-col cols="12" md="6">
                        <v-card class="elevation-12 rounded-lg">
                            <v-card-text class="pa-5 mb-5">
                                <h2 class="text-center mb-10 black--text font-weight-light">EDITAR PAÍS</h2>
                                
                                <v-form ref="form">
                                    <div class="mt-5 mb-5">
                                        <div class="d-flex align-center mb-2">
                                            <font-awesome-icon :icon="['fas', 'flag']" class="black--text mr-3" />
                                            <label class="label-large black--text">Nombre</label>
                                        </div>
                                        <v-text-field v-model="pais.nombre" outlined
                                            class="black--text flex-grow-1" :rules="[rules.required]">
                                        </v-text-field>
                                    </div>
                                    <div class="mt-5 mb-5">
                                        <div class="d-flex align-center mb-2">
                                            <font-awesome-icon :icon="['fas', 'building']" class="black--text mr-3" />
                                            <label class="label-large black--text">Capital</label>
                                        </div>
                                        <v-text-field v-model="pais.capital" outlined
                                            class="black--text flex-grow-1" :rules="[rules.required]">
                                        </v-text-field>
                                    </div>
                                    <v-row justify="center">
                                        <v-col cols="8" md="4">
                                            <v-btn :color="buttonColor" rounded large block @click="updatePais"
...
export default {
    ...
    methods: {
        async fetchPais() {
            try {
                const response = await api.get(`/pais/${this.id}`);
        ...
        async updatePais() {
            ...
            if (this.$refs.form.validate()) {
                ...
                try {
                    const response = await api.put(`/pais/${this.id}`, requestData);
                    if (response.data.success) {
                        this.success = 'País actualizado exitosamente';
                        this.$router.go(-1);
                    } else {
                        this.error = response.data.error || 'Error al actualizar el país';
                    }
                } catch (error) {
